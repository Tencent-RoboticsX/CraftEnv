...
class CraftEnv(MatrixEnv):
    ...
    def _init_obs_and_ac_dim(self):
        ...
        if enable_local_obs:
            ...
            self.ob_dim = self._ob_dim * (obj_num +
                                          blackboard.smartcar_num + 1) + \
        ...
        for i, j, k in itertools.product(range(self.area_size[0]),
            ...
            if obj.type is ObjType.FoldedSlope:
                ...
                if obj.near_unfold_slope_body:
                    ...
                    # calculate the building complexity score
                    current_building_complexity_score = 0
                    origin_block_list.append([i, j, k])
                    while index < len(origin_block_list):
                        i = origin_block_list[index][0]
                        j = origin_block_list[index][1]
                        k = origin_block_list[index][2]
                        obj_origin = grid[i][j][k]
                        obj_left = grid[i - 1][j][k]
                        obj_right = grid[i + 1][j][k]
                        obj_front = grid[i][j + 1][k]
                        obj_back = grid[i][j - 1][k]
                        obj_up = grid[i][j][k + 1]
                        obj_down = grid[i][j][k - 1]
                        if obj_origin.type is ObjType.Block:
                            if not ([i, j, k] in seen_obj_list):
                                seen_obj_list.append([i, j, k])
                                current_building_complexity_score += 1 * \
                                    self.env_config['complexity_1']
                        if obj_left.type is ObjType.Block:
                            if not ([i - 1, j, k] in origin_block_list):
                                origin_block_list.append([i - 1, j, k])
                            if not ([i - 1, j, k] in seen_obj_list):
                                seen_obj_list.append([i - 1, j, k])
                                current_building_complexity_score += 1 * \
                                    self.env_config['complexity_1']
                        if obj_left.type is ObjType.UnfoldedSlopeBody:
                            if not ([i - 1, j, k] in seen_obj_list):
                                seen_obj_list.append([i - 1, j, k])
                                current_building_complexity_score += 1 * \
                                    self.env_config['complexity_1']
                        if obj_right.type is ObjType.Block:
                            if not ([i + 1, j, k] in origin_block_list):
                                origin_block_list.append([i + 1, j, k])
                            if not ([i + 1, j, k] in seen_obj_list):
                                seen_obj_list.append([i + 1, j, k])
                                current_building_complexity_score += 1 * \
                                    self.env_config['complexity_1']
                        if obj_right.type is ObjType.UnfoldedSlopeBody:
                            if not ([i + 1, j, k] in seen_obj_list):
                                seen_obj_list.append([i + 1, j, k])
                                current_building_complexity_score += 1 * \
                                    self.env_config['complexity_1']
                        if obj_front.type is ObjType.Block:
                            if not ([i, j + 1, k] in origin_block_list):
                                origin_block_list.append([i, j + 1, k])
                            if not ([i, j + 1, k] in seen_obj_list):
                                seen_obj_list.append([i, j + 1, k])
                                current_building_complexity_score += 1 * \
                                    self.env_config['complexity_1']
                        if obj_front.type is ObjType.UnfoldedSlopeBody:
                            if not ([i, j + 1, k] in seen_obj_list):
                                seen_obj_list.append([i, j + 1, k])
                                current_building_complexity_score += 1 * \
                                    self.env_config['complexity_1']
                        if obj_back.type is ObjType.Block:
                            if not ([i, j - 1, k] in origin_block_list):
                                origin_block_list.append([i, j - 1, k])
                            if not ([i, j - 1, k] in seen_obj_list):
                                seen_obj_list.append([i, j - 1, k])
                                current_building_complexity_score += 1 * \
                                    self.env_config['complexity_1']
                        if obj_back.type is ObjType.UnfoldedSlopeBody:
                            if not ([i, j - 1, k] in seen_obj_list):
                                seen_obj_list.append([i, j - 1, k])
                                current_building_complexity_score += 1 * \
                                    self.env_config['complexity_1']
                        if obj_up.type is ObjType.Block:
                            if not ([i, j, k + 1] in origin_block_list):
                                origin_block_list.append([i, j, k + 1])
                            if not ([i, j, k + 1] in seen_obj_list):
                                seen_obj_list.append([i, j, k + 1])
                                current_building_complexity_score += 1 * \
                                    self.env_config['complexity_2']
                                if k == 2:
                                    current_building_complexity_score += 1 * \
                                        self.env_config['complexity_3']
                        if obj_up.type is ObjType.UnfoldedSlopeBody:
                            if not ([i, j, k + 1] in seen_obj_list):
                                seen_obj_list.append([i, j, k + 1])
                                current_building_complexity_score += 1 * \
                                    self.env_config['complexity_4']
                            if (grid[i - 1][j][k + 1].type is ObjType.Block
                                    and grid[i - 1][j][k + 1].
                                near_unfold_slope_body)\
                                    or (grid[i + 1][j][k + 1].type is
                                        ObjType.Block
                                        and grid[i + 1][j][k + 1].
                                        near_unfold_slope_body)\
                                    or (grid[i][j - 1][k + 1].type is
                                        ObjType.Block
                                        and grid[i][j - 1][k + 1].
                                        near_unfold_slope_body)\
                                    or (grid[i][j + 1][k + 1].type
                                        is ObjType.Block
                                        and grid[i][j + 1][k + 1].
                                        near_unfold_slope_body):
                                current_building_complexity_score += 1 * \
                                    self.env_config['complexity_5']
                                if (grid[i - 2][j][k].type is ObjType.Block
                                    and grid[i - 2][j][k].
                                    near_blow_unfold_slope_foot)\
                                    or (grid[i + 2][j][k].type is ObjType.Block
                                        and grid[i + 2][j][k].
                                        near_blow_unfold_slope_foot)\
                                    or (grid[i][j - 2][k].type is ObjType.Block
                                        and grid[i][j - 2][k].
                                        near_blow_unfold_slope_foot)\
                                    or (grid[i][j + 2][k].type is ObjType.Block
                                        and grid[i][j + 2][k].
                                    ...
                                    if (grid[i - 2][j][k].
                                            ...
                                            or (grid[i][j + 2][k].
                                                near_unfold_slope_body):
                                        current_building_complexity_score\
                                            += 1 * \
                                            self.env_config['complexity_6']
                        if obj_down.type is ObjType.Block:
                            if not ([i, j, k - 1] in origin_block_list):
                                origin_block_list.append([i, j, k - 1])
                            if not ([i, j, k - 1] in seen_obj_list):
                                seen_obj_list.append([i, j, k - 1])
                                current_building_complexity_score += 1 * \
                                    self.env_config['complexity_1']
                        if obj_down.type is ObjType.UnfoldedSlopeBody:
                            if not ([i, j, k - 1] in seen_obj_list):
                                seen_obj_list.append([i, j, k - 1])
                                current_building_complexity_score += 1 * \
                                    self.env_config['complexity_1']
                        index += 1
                    if current_building_complexity_score > 0:
                        self.building_complexity_score_list.append(
                            current_building_complexity_score)

            elif obj.type is ObjType.Flag:
                obs_flag = self.get_object_vector(obj.type, i, j, k)
                self.flag_pos_x = i
                self.flag_pos_y = j
                self.flag_pos_z = k
        if (len(self.building_complexity_score_list) > 0):
            self.building_complexity_score_max =\
                min(max(self.building_complexity_score_list),
                    self.env_config['building_complexity_max'])
